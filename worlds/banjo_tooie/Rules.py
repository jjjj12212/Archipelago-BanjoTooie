...
class BanjoTooieRules:
    ...
    def __init__(self, world: BanjoTooieWorld) -> None:
        ...
        self.cheato_rules = {
            ...
            locationName.CHEATOHP2: lambda state: self.hasBKMove(state, itemName.CLIMB) and (state.has(itemName.CEGGS, self.player) or self.check_solo_moves(state, itemName.SHPACK)),
        ...
        self.honey_rules = {
            ...
            locationName.HONEYCCC2: lambda state: self.cclStarterPack(state) and self.hasBKMove(state, itemName.FPAD),
            locationName.HONEYCCC3: lambda state: self.cclStarterPack(state) and self.hasBKMove(state, itemName.FPAD),
        ...
        self.glowbo_rules = {
            ...
            locationName.GLOWBOCC1: lambda state: self.hasBKMove(state, itemName.DIVE),
        ...
        self.silo_rules = {
            ...
            locationName.SAPACK: lambda state: self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SHPACK) and self.check_notes(state, 765),
        ...
        self.jinjo_rules = {
            ...
            locationName.JINJOGM4: lambda state: self.humbaGGM(state) or (self.hasBKMove(state, itemName.TTROT) and self.billDrill(state)),
            ...
            locationName.JINJOCC4: lambda state: self.cclStarterPack(state),
            locationName.JINJOCC5: lambda state: self.hasBKMove(state, itemName.CLIMB),
        }
        ...
        self.notes_rules = {
            ...
            locationName.NOTETDL13:  lambda state: self.hasBKMove(state, itemName.DIVE),
            locationName.NOTETDL14:  lambda state: self.hasBKMove(state, itemName.DIVE),
            locationName.NOTETDL15:  lambda state: self.hasBKMove(state, itemName.DIVE),
            locationName.NOTETDL16:  lambda state: self.hasBKMove(state, itemName.DIVE),
            ...
            locationName.NOTEHFP3:  lambda state: self.hfpTop(state),
            locationName.NOTEHFP4:  lambda state: self.hfpTop(state),
            ...
            locationName.NOTEHFP9:  lambda state: self.hfpTop(state),
            ...
            locationName.NOTECCL7: lambda state: self.cclStarterPack(state) and self.hasBKMove(state, itemName.DIVE),
            ...
            locationName.NOTECCL13: lambda state: self.hasBKMove(state, itemName.DIVE),
    ...
    def jiggy_cactus(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.billDrill(state) and self.canShootEggs(state, itemName.GEGGS)\
                  and self.hasBKMove(state, itemName.BBUST) and (self.hasBKMove(state, itemName.CLIMB) or self.clockwork_shot(state))
        elif self.world.options.logic_type == 3: # glitched
            logic = self.billDrill(state) and self.canShootEggs(state, itemName.GEGGS)\
                  and self.hasBKMove(state, itemName.BBUST) and (self.hasBKMove(state, itemName.CLIMB) or self.clockwork_shot(state))
        return logic
    ...
    def jiggy_dippy(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = state.has(itemName.TTORP, self.player) and self.can_access_ccl(state) and self.cclStarterPack(state) and self.hasBKMove(state, itemName.DIVE)
        elif self.world.options.logic_type == 1: # normal
            logic = state.has(itemName.TTORP, self.player) and self.can_access_ccl(state) and self.cclStarterPack(state) and self.hasBKMove(state, itemName.DIVE)
        elif self.world.options.logic_type == 2: # advanced
            logic = state.has(itemName.TTORP, self.player) and self.can_access_ccl(state) and self.cclStarterPack(state) and self.hasBKMove(state, itemName.DIVE)
    ...
    def jiggy_scrotty(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            logic = self.canShootEggs(state, itemName.GEGGS) and self.WW_train_station(state) and \
                    ...
                    state.has(itemName.EGGAIM, self.player) and self.hasBKMove(state, itemName.TTROT)
        elif self.world.options.logic_type == 1: # normal
            logic = self.canShootEggs(state, itemName.GEGGS) and self.WW_train_station(state) and \
                    self.CT_train_station(state) and state.has(itemName.TRAINSWTD, self.player) and \
                    self.check_solo_moves(state, itemName.TAXPACK) and self.check_mumbo_magic(state, itemName.MUMBOIH) and \
    ...
    def jiggy_stomping_plains(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.canShootEggs(state, itemName.IEGGS) and state.has(itemName.SPRINGB, self.player) and self.longJump(state)
        elif self.world.options.logic_type == 1: # normal
            ...
            self.canShootEggs(state, itemName.IEGGS)) and self.longJump(state)
        elif self.world.options.logic_type == 2: # advanced
            logic = state.has(itemName.SPRINGB, self.player) and self.longJump(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = (state.has(itemName.SPRINGB, self.player) or (self.hasBKMove(state, itemName.FPAD) and (self.hasBKMove(state, itemName.BBOMB) or self.clockworkWarp(state)))) and self.longJump(state)
        return logic
    ...
    def jiggy_skivvy(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.check_humba_magic(state, itemName.HUMBAGI) and self.billDrill(state)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.check_humba_magic(state, itemName.HUMBAGI) and self.billDrill(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = self.check_humba_magic(state, itemName.HUMBAGI) and self.billDrill(state)
    ...
    def jiggy_floor5(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = state.has(itemName.SPLITUP, self.player) or self.clockwork_shot(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = state.has(itemName.SPLITUP, self.player) or self.clockwork_shot(state)
        return logic
    ...
    def jiggy_qa(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.canShootEggs(state, itemName.GEGGS) and\
                  ((self.mumboGI(state) and self.can_use_battery(state)) or (state.has(itemName.CEGGS, self.player)))
        return logic
    ...
    def jiggy_dragons_bros(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            logic = self.canShootEggs(state, itemName.FEGGS) and self.canShootEggs(state, itemName.IEGGS) and \
                    state.has(itemName.CLAWBTS, self.player) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 1: # normal
            logic = self.canShootEggs(state, itemName.FEGGS) and self.canShootEggs(state, itemName.IEGGS) and \
                    state.has(itemName.CLAWBTS, self.player) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 2: # advanced
            ...
            logic = self.canShootEggs(state, itemName.FEGGS) and self.canShootEggs(state, itemName.IEGGS) and \
                    self.check_solo_moves(state, itemName.PACKWH) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 3: # glitched
            # Some people are going to fight Chilly Willy without Claw Clamber Boots, so to prevent softlocks, we also require Pack Whack.
            logic = self.canShootEggs(state, itemName.FEGGS) and self.canShootEggs(state, itemName.IEGGS) and \
                    self.check_solo_moves(state, itemName.PACKWH) and self.hasBKMove(state, itemName.FPAD)
    ...
    def jiggy_boggy(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.hfpTop(state) and self.canDoSmallElevation(state) and (self.check_solo_moves(state, itemName.SHPACK) or (state.has(itemName.CEGGS, self.player) and self.hasBKMove(state, itemName.EGGSHOOT)) or self.check_solo_moves(state, itemName.LSPRING))
        return logic
    ...
    def jiggy_hfp_stomping(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.jinjo_stomping_plains(state) and self.check_solo_moves(state, itemName.SNPACK)
        elif self.world.options.logic_type == 1: # normal
            logic = self.jinjo_stomping_plains(state) and self.check_solo_moves(state, itemName.SNPACK)
        elif self.world.options.logic_type == 2: # advanced
    ...
    def jiggy_aliens(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.jiggy_ufo(state) and self.can_access_jrl(state) and self.billDrill(state) and \
                    self.check_solo_moves(state, itemName.HATCH) and self.check_solo_moves(state, itemName.GLIDE) and \
                    self.check_mumbo_magic(state, itemName.MUMBOHP)
        elif self.world.options.logic_type == 1: # normal
            logic = self.jiggy_ufo(state) and self.can_access_jrl(state) and self.check_mumbo_magic(state, itemName.MUMBOHP) and \
    ...
    def jiggy_colosseum_split(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = (state.has(itemName.SPLITUP, self.player) and (state.has(itemName.GGRAB, self.player) or (self.check_solo_moves(state, itemName.PACKWH) and self.hasBKMove(state, itemName.TJUMP))))\
    ...
    def jiggy_mingy(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.hasMobileAttack(state) and self.cclStarterPack(state)
        elif self.world.options.logic_type == 1: # normal
            logic = self.hasMobileAttack(state) and self.cclStarterPack(state)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.hasGroundAttack(state) and self.cclStarterPack(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = self.hasGroundAttack(state) and self.cclStarterPack(state)
        return logic
    ...
    def jiggy_cheese(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SAPACK) and self.grow_beanstalk(state) and \
    ...
    def jiggy_trash(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state) and self.check_solo_moves(state, itemName.WWHACK) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 1: # normal
            logic = self.cclStarterPack(state) and state.has(itemName.SPLITUP, self.player)\
                and (self.check_solo_moves(state, itemName.WWHACK) or self.canShootLinearEgg(state)) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.cclStarterPack(state) and state.has(itemName.SPLITUP, self.player)\
                and (self.check_solo_moves(state, itemName.WWHACK) or self.canShootLinearEgg(state)) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 3: # glitched
    ...
    def jiggy_sstash(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.cclStarterPack(state) and self.canShootEggs(state, itemName.CEGGS)
        return logic
    ...
    def honeycomb_prospector(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = (self.hasBKMove(state, itemName.FFLIP) or self.GGMSlope(state) or self.canReachSlightlyElevatedLedge(state)) and self.billDrill(state)\
            ...
            logic = (self.hasBKMove(state, itemName.FFLIP) or self.GGMSlope(state) or self.canReachSlightlyElevatedLedge(state) and self.billDrill(state)) or \
    ...
    def honeycomb_spacezone(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = (state.has(itemName.GGRAB, self.player) and self.hasBKMove(state, itemName.BBUST)) and self.hasBKMove(state, itemName.CLIMB) and self.hasBKMove(state, itemName.FFLIP)
        elif self.world.options.logic_type == 3: # glitched
            logic = ((state.has(itemName.GGRAB, self.player) and self.hasBKMove(state, itemName.BBUST)) and self.hasBKMove(state, itemName.CLIMB) and self.hasBKMove(state, itemName.FFLIP))\
                    or (self.clockwork_shot(state) and self.can_access_GM(state) and self.ggm_to_ww(state))
        return logic
    ...
    def honeycomb_jrlpipes(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            logic = (self.has_explosives(state) or  self.billDrill(state)) and \
                    state.has(itemName.GGRAB, self.player) and self.springPad(state)
        elif self.world.options.logic_type == 1: # normal
            logic = ((self.has_explosives(state) or self.billDrill(state))\
                    and state.has(itemName.GGRAB, self.player) and self.springPad(state))\
    ...
    def honeycomb_river(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.hasBKMove(state, itemName.TTROT) or self.clockwork_shot(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = self.hasBKMove(state, itemName.TTROT) or self.clockwork_shot(state)
        return logic
    ...
    def honeycomb_volcano(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.hfpTop(state) and state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player)
        elif self.world.options.logic_type == 1: # normal
            logic = self.hfpTop(state) and state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player) or \
                    state.has(itemName.SPLITUP, self.player)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.hfpTop(state) and state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player) or \
                    state.has(itemName.SPLITUP, self.player)
    ...
    def honeycomb_hfpstation(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.canReachSlightlyElevatedLedge(state) or self.check_solo_moves(state, itemName.LSPRING) or \
    ...
    def cheato_dippypool(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.can_access_ccl(state) and self.jiggy_dippy(state) and self.hasBKMove(state, itemName.DIVE)\
    ...
    def cheato_icypillar(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            logic = state.has(itemName.SPLITUP, self.player) and state.has(itemName.GEGGS, self.player) and \
                    state.has(itemName.EGGAIM, self.player) and (self.hasBKMove(state, itemName.TJUMP) or state.has(itemName.WWHACK, self.player) or state.has(itemName.GLIDE, self.player))
        elif self.world.options.logic_type == 1: # normal
            logic = state.has(itemName.SPLITUP, self.player) and (self.hasBKMove(state, itemName.TJUMP) or state.has(itemName.WWHACK, self.player) or state.has(itemName.GLIDE, self.player))\
                    or state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player)
        elif self.world.options.logic_type == 2: # advanced
            logic = state.has(itemName.SPLITUP, self.player) and (self.hasBKMove(state, itemName.TJUMP) or state.has(itemName.WWHACK, self.player) or state.has(itemName.GLIDE, self.player))\
                    or state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player)
        elif self.world.options.logic_type == 3: # glitched
    ...
    def glowbo_cliff(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.hasBKMove(state, itemName.CLIMB) or (self.clockwork_shot(state) and self.can_access_clifftop(state, False))
        elif self.world.options.logic_type == 3: # glitched
            logic = self.hasBKMove(state, itemName.CLIMB) or (self.clockwork_shot(state) and self.can_access_clifftop(state, False))
        return logic
    ...
    def mega_glowbo(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = state.has(itemName.TTORP, self.player) and state.has(itemName.IKEY, self.player)\
                    ...
                    (self.hasBKMove(state, itemName.TJUMP) or self.clockwork_shot(state))
        elif self.world.options.logic_type == 3: # glitched
            logic = state.has(itemName.TTORP, self.player) and state.has(itemName.IKEY, self.player)\
                    and self.hasBKMove(state, itemName.DIVE) and\
                    (self.hasBKMove(state, itemName.TJUMP) or self.clockwork_shot(state))
    ...
    def jinjo_tent(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.hasBKMove(state, itemName.TTROT) or self.humbaWW(state)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.hasBKMove(state, itemName.TTROT) or self.humbaWW(state) or self.clockwork_shot(state) or\
                  ((state.has(itemName.GGRAB, self.player) and self.hasBKMove(state, itemName.BBUST)) and self.hasBKMove(state, itemName.CLIMB) and self.hasBKMove(state, itemName.FFLIP))
        elif self.world.options.logic_type == 3: # glitched
            logic = self.hasBKMove(state, itemName.TTROT) or self.humbaWW(state) or self.clockwork_shot(state) or\
                  ((state.has(itemName.GGRAB, self.player) and self.hasBKMove(state, itemName.BBUST)) and self.hasBKMove(state, itemName.CLIMB) and self.hasBKMove(state, itemName.FFLIP))
        return logic
    ...
    def jinjo_icegrotto(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.check_solo_moves(state, itemName.GLIDE) or (self.check_solo_moves(state, itemName.LSPRING) and 
                    self.check_solo_moves(state, itemName.WWHACK)) or self.clockwork_shot(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = self.check_solo_moves(state, itemName.GLIDE) or (self.check_solo_moves(state, itemName.LSPRING) and 
                    self.check_solo_moves(state, itemName.WWHACK)) or self.clockwork_shot(state)
        return logic
    ...
    def jinjo_mildred(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.canShootEggs(state, itemName.FEGGS) or self.has_explosives(state) or self.billDrill(state) or \
                    self.check_mumbo_magic(state, itemName.MUMBOHP)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.canShootEggs(state, itemName.FEGGS) or self.has_explosives(state) or self.billDrill(state) or \
                    self.check_mumbo_magic(state, itemName.MUMBOHP)
    ...
    def jinjo_trashcan(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SHPACK) and self.hasBKMove(state, itemName.CLIMB)
        elif self.world.options.logic_type == 1: # normal
            logic =  self.cclStarterPack(state) and ((self.check_solo_moves(state, itemName.SHPACK) and self.hasBKMove(state, itemName.CLIMB)) or self.check_solo_moves(state, itemName.LSPRING))
        elif self.world.options.logic_type == 2: # advanced
            logic =   self.cclStarterPack(state) and ((self.check_solo_moves(state, itemName.SHPACK) and self.hasBKMove(state, itemName.CLIMB)) or self.check_solo_moves(state, itemName.LSPRING) or self.check_solo_moves(state, itemName.GLIDE)\
    ...
    def jinjo_cheese(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state) and self.hasBKMove(state, itemName.FPAD) and (self.check_solo_moves(state, itemName.SAPACK) and self.grow_beanstalk(state) and \
    ...
    def jinjo_central(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.cclStarterPack(state) and (state.has(itemName.SPLITUP, self.player) or (state.has(itemName.SPRINGB, self.player) and self.billDrill(state)))
        elif self.world.options.logic_type == 2: # advanced
            logic = self.cclStarterPack(state) and (state.has(itemName.SPLITUP, self.player)\
                  or self.clockwork_shot(state)\
    ...
    def treble_hfp(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = (state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player) and self.hasBKMove(state, itemName.TJUMP))
        elif self.world.options.logic_type == 1: # normal
            ...
            logic = (state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player)) or state.has(itemName.SPLITUP, self.player)\
                  or self.clockwork_shot(state) or self.extremelyLongJump(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = (state.has(itemName.GEGGS, self.player) and state.has(itemName.EGGAIM, self.player)) or state.has(itemName.SPLITUP, self.player)\
                  or self.clockwork_shot(state) or self.extremelyLongJump(state)
        return logic
    ...
    def notes_ww_area51(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = ((self.has_explosives(state) or state.has(itemName.SPLITUP, self.player)) and self.springPad(state)) or self.clockwork_shot(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = ((self.has_explosives(state) or state.has(itemName.SPLITUP, self.player)) and self.springPad(state)) or self.clockwork_shot(state)
        return logic
    ...
    def notes_jolly(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.canDoSmallElevation(state) or self.clockwork_shot(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = self.canDoSmallElevation(state) or self.clockwork_shot(state)
        return logic
    ...
    def notes_ccl_silo(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SHPACK)
        elif self.world.options.logic_type == 1: # normal
            logic = (self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SHPACK)) or self.canShootEggs(state, itemName.CEGGS)
        elif self.world.options.logic_type == 2: # advanced
            logic = (self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SHPACK)) or self.canShootEggs(state, itemName.CEGGS)
        elif self.world.options.logic_type == 3: # glitched
            logic = (self.cclStarterPack(state) and self.check_solo_moves(state, itemName.SHPACK)) or self.canShootEggs(state, itemName.CEGGS)
    ...
    def notes_cheese(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = (self.cclStarterPack(state) and self.hasBKMove(state, itemName.CLIMB) and self.check_solo_moves(state, itemName.SAPACK))\
            ...
            logic = (self.cclStarterPack(state) and self.hasBKMove(state, itemName.CLIMB) and self.check_solo_moves(state, itemName.SAPACK))\
                    or (self.cclStarterPack(state) and state.has(itemName.SPRINGB, self.player))\
                    or self.notes_ccl_high(state)\
                    or self.clockwork_shot(state)
        elif self.world.options.logic_type == 3: # glitched
            logic = (self.cclStarterPack(state) and self.hasBKMove(state, itemName.CLIMB) and self.check_solo_moves(state, itemName.SAPACK))\
    ...
    def notes_ccl_high(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 1: # normal
            logic = self.cclStarterPack(state) and self.hasBKMove(state, itemName.FPAD)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.cclStarterPack(state) and self.hasBKMove(state, itemName.FPAD)
    ...
    def notes_ccl_low(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.cclStarterPack(state)
        elif self.world.options.logic_type == 1: # normal
            logic = self.cclStarterPack(state)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.cclStarterPack(state)
    ...
    def can_beat_terry(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            return state.has(itemName.SPRINGB, self.player) and self.canShootLinearEgg(state) and\
                (self.hasBKMove(state, itemName.FFLIP) or state.has(itemName.EGGAIM, self.player))
        elif self.world.options.logic_type == 3: # glitched
            return self.canShootEggs(state) and \
                    (self.hasBKMove(state, itemName.FFLIP) or state.has(itemName.EGGAIM, self.player)) and\
                    (state.has(itemName.SPRINGB, self.player) or \
                    (self.hasBKMove(state, itemName.FPAD) and (self.hasBKMove(state, itemName.BBOMB) or\
                    state.has(itemName.EGGAIM, self.player) and self.hasBKMove(state, itemName.EGGSHOOT) and state.has(itemName.GEGGS, self.player) and state.has(itemName.CEGGS, self.player))))
    ...
    def jiggy_clinkers(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = self.mumboGI(state) and state.has(itemName.CLAWBTS, self.player) and state.has(itemName.BBLASTER, self.player)
        elif self.world.options.logic_type == 1: # normal
            logic = self.mumboGI(state) and state.has(itemName.CLAWBTS, self.player) and state.has(itemName.BBLASTER, self.player)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.mumboGI(state) and state.has(itemName.CLAWBTS, self.player) and state.has(itemName.BBLASTER, self.player)
        elif self.world.options.logic_type == 3: # glitched
            logic = (self.mumboGI(state) or (self.breegullBash(state) and self.hasBKMove(state, itemName.CLIMB)))\
    ...
    def HFP_to_JRL(self, state: CollectionState) -> bool:
        ...
        if self.world.options.logic_type == 0: # beginner
            ...
            logic = self.HFP_hot_water_cooled(state) or \
                (state.has(itemName.GGRAB, self.player) and self.hasBKMove(state, itemName.FLUTTER) and self.hasBKMove(state, itemName.GRAT))
        return logic
    ...
    def humbaTDL(self, state: CollectionState) -> bool:
        logic = True
        if self.world.options.logic_type == 0: # beginner
            logic = (self.canDoSmallElevation(state) or self.TDLFlightPad(state)) and state.has(itemName.HUMBATD, self.player)
        elif self.world.options.logic_type == 1: # normal
            logic = (self.canDoSmallElevation(state) or self.TDLFlightPad(state)) and state.has(itemName.HUMBATD, self.player)
        elif self.world.options.logic_type == 2: # advanced
            logic = self.check_humba_magic(state, itemName.HUMBATD) and (self.canDoSmallElevation(state) or self.TDLFlightPad(state) or self.clockwork_shot(state))
        elif self.world.options.logic_type == 3: # glitched
            logic = self.check_humba_magic(state, itemName.HUMBATD) and (self.canDoSmallElevation(state) or self.TDLFlightPad(state) or self.clockwork_shot(state))
        return logic
    ...
    def GGMSlope(self, state: CollectionState) -> bool:
        return self.hasBKMove(state, itemName.TTROT) or self.hasBKMove(state, itemName.TTRAIN)\
              or (self.GM_boulders(state) and state.has(itemName.SPLITUP, self.player))
    ...
    # TODO: Check is HFP is entered from the train station. I don't know how to do that.
