...
local WARP_TABLE = {
    [1]  = {mapId = 0xAF, worldName = "Spiral Mountain", Xpos = 1, Ypos = 1850, Zpos = 1}, -- notes: CCL cavern, by the treble clef
}

local MORPH_TABLE = {
    [1]  = {name = "BK", morph = 1},
    [2]  = {name = "Snowball", morph = 2},
    [3]  = {name = "Bee", morph = 6},
    [4]  = {name = "W. Machine", morph = 7},
    [5]  = {name = "Stony", morph = 8},
    [6]  = {name = "Banjo", morph = 10},
    [7]  = {name = "Kazooie", morph = 11},
    [8]  = {name = "Mumbo", morph = 13},
    [9]  = {name = "Detonator", morph = 15},
    [10]  = {name = "Van", morph = 16},
    [11]  = {name = "Small T-Rex", morph = 18},
}

local MORPH_COUNTER = 0
local CURRENT_MORPH = false
local PREVIOUS_STATE = 1
local TEST_MORPH = false
...
BTRAM = {
    ...
    banjoAnimationPointer = 0x1A0,
    animation_state = 0x34,
    movement_ptr = 0x120,
    current_state = 0x4,
    map_dest = 0x045702,
...
function BTRAM:setBanjoPos(Xnew, Ynew, Znew)
    local banjo = BTRAM:banjoPTR()
    if banjo == nil
    then
        return false;
...
function BTRAM:setBanjoAnimation(animationID)
    local banjo = BTRAM:banjoPTR()
    if banjo == nil
    then
        return false;
    end
    local plptr = BTRAM:dereferencePointer(banjo + self.player_vel_ptr);
    if plptr == nil
    then
        return false;
    end
	if Xvel ~= nil
    then
        mainmemory.writefloat(plptr + 0x10, animationID, true);
    end
end

function BTRAM:setXVel(Xvel)
    local banjo = BTRAM:banjoPTR()
    if banjo == nil
    then
        return false;
	...
	if Xvel ~= nil
    then
        mainmemory.writefloat(plptr + 0x10, Xvel, true);
    end
end

function BTRAM:setYVel(Yvel)
    local banjo = BTRAM:banjoPTR()
    if banjo == nil
    then
        return false;
	...
	if Yvel ~= nil
    then
        mainmemory.writefloat(plptr + 0x14, Yvel, true);
    end
end

function BTRAM:setZVel(Zvel)
    local banjo = BTRAM:banjoPTR()
    if banjo == nil
    then
        return false;
	...
	if Zvel ~= nil
    then
        mainmemory.writefloat(plptr + 0x18, Zvel, true);
    end
end

function BTRAM:getBanjoState()
    return mainmemory.readbyte(self.character_state);
end

function BTRAM:setBanjoState(state)
    mainmemory.writebyte(self.character_state, state);
end

function BTRAM:characterChange(morph)
    mainmemory.writebyte(self.character_change, morph)
    print("in character change now")
    print(morph)
end
...
function BTRAM:setBanjoMovementState(state)
    local player = BTRAM:banjoPTR()
    if player ~= nil
    then
        local movestate = BTRAM:dereferencePointer(player + self.movement_ptr)
        mainmemory.write_u32_be(movestate + self.current_state, state)
    end
    return nil
end


function BTRAM:getBanjoAnimation()
    local player = BTRAM:banjoPTR()
    if player ~= nil
    then
        local movestate = BTRAM:dereferencePointer(player + self.banjoAnimationPointer)
        return mainmemory.read_u32_be(movestate + self.animation_state)
    end
    return nil
end

function BTRAM:setBanjoAnimation(animation)
    local player = BTRAM:banjoPTR()
    if player ~= nil
    then
        local animationState = BTRAM:dereferencePointer(player + self.banjoAnimationPointer)
        mainmemory.write_u32_be(animationState + self.animation_state, animation)
    end
    return nil
end
...
function PositionAverage(start, destination, scalar)
    local average  = {
        Xpos = 0,
        Ypos = 0,
        Zpos = 0
    }
    local newXpos = (((destination["Xpos"]-start["Xpos"])*scalar)+start["Xpos"])
    local newYpos = (((destination["Ypos"]-start["Ypos"])*scalar)+start["Ypos"])
    local newZpos = (((destination["Zpos"]-start["Zpos"])*scalar)+start["Zpos"])
    average  = {
        Xpos = newXpos,
        Ypos = newYpos,
        Zpos = newZpos
...
function WarpTrap()
    ...
    --then
            ...
            local currentPos = BTModel.banjoRAM:getBanjoPos()
            local slowWarp = -.1
            local avgPos = PositionAverage(currentPos, warpLocation, slowWarp)
            local safetyHeight = warpLocation["Ypos"]+100
        --if (mapValue == CURRENT_MAP) -- and (WARP_TRAP_LOGIC == 1)
 		--then
        

            BTRAMOBJ:setFlag(0xA1, 6, "Fall Damage")
            --BTModel.banjoRAM:setBanjoAnimation(0x68)
            while slowWarp <=1
            do
                if currentPos["Xpos"] > warpLocation["Xpos"]
                then
                    BTModel.banjoRAM:setXVel(-5000)
                end
                if currentPos["Xpos"] < warpLocation["Xpos"]
                then
                    BTModel.banjoRAM:setXVel(5000)
                end
                if currentPos["Ypos"] > safetyHeight
                then
                    BTModel.banjoRAM:setYVel(-5000)
                end
                if currentPos["Ypos"] < safetyHeight
                then
                    BTModel.banjoRAM:setYVel(5000)
                end
                if currentPos["Zpos"] > warpLocation["Zpos"]
                then
                    BTModel.banjoRAM:setZVel(-5000)
                end
                if currentPos["Zpos"] < warpLocation["Zpos"]
                then
                    BTModel.banjoRAM:setZVel(5000)
                end
                --avgPos = PositionAverage(currentPos, warpLocation, slowWarp)
                --BTModel.banjoRAM:setBanjoPos(avgPos["Xpos"], safetyHeight, avgPos["Zpos"]);
                currentPos = BTModel.banjoRAM:getBanjoPos()
                slowWarp = slowWarp + .005
                emu.frameadvance()
            end
            if slowWarp > 1
            then
                BTModel.banjoRAM:setXVel(0)
                BTModel.banjoRAM:setYVel(0)
                BTModel.banjoRAM:setZVel(0)
                BTModel.banjoRAM:setBanjoPos(warpLocation["Xpos"], safetyHeight, warpLocation["Zpos"]);
                emu.frameadvance()
                emu.frameadvance()
                emu.frameadvance()
                emu.frameadvance()
...
function MorphTrap()
    if CURRENT_MORPH ~= true and TEST_MORPH == true--and TOTAL_MORPH_COUNTER < AGI_TRAPS["1230800"]
    then
        PREVIOUS_STATE = BTModel.banjoRAM:getBanjoState()
        local morphIndex = math.random(2, #MORPH_TABLE)
        BTRAMOBJ:characterChange(MORPH_TABLE[morphIndex]["morph"])
        CURRENT_MORPH = true
        MORPH_COUNTER = 3
        TEST_MORPH = false
        --TOTAL_MORPH_COUNTER = AGI_TRAPS["1230800"];
    end
    -- if CURRENT_MORPH == true and MORPH_COUNTER <= 0
    -- then
    --     BTRAMOBJ:characterChange(PREVIOUS_STATE)
    --     CURRENT_MORPH = false
    -- end
end
...
function BKLogics(mapaddr)
    ...
    if ((CURRENT_MAP ~= mapaddr) or player == false) and MORPH_COUNTER > 0
    then
...
function main()
    ...
    -- BTModel.banjoRAM:setBanjoMovementState(0)
    -- WarpTrap()
    -- BTModel.banjoRAM:setBanjoMovementState(1)
    while true do
        FRAME = FRAME + 1
        ...
        if true == true then
            if (FRAME % 60 == 1) then
                ...
                then
                    BTModel.banjoRAM:setBanjoAnimation(0x14)
                end
                if check_controls ~= nil and check_controls['P1 DPad L'] == true
                then
                    BTModel.banjoRAM:setBanjoAnimation(0x46)
                end
                if check_controls ~= nil and check_controls['P1 DPad R'] == true
                then
                    BTModel.banjoRAM:setBanjoAnimation(0x3e)
                end
            elseif (FRAME % 10 == 1)
            then
                -- BTRAMOBJ:characterChange(6)
                checkPause();
                checkTotalMenu();
